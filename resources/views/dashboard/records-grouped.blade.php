...
<head>
    ...
    <style>
        ...
        /* Light mode styles for records-grouped */
        html:not(.dark) body {
            @apply bg-gray-50 text-gray-900;
        }

        html:not(.dark) [class*="bg-\[#0b0f1a\]"],
        html:not(.dark) [class*="bg-\[#131928\]"],
        html:not(.dark) [class*="bg-\[#0f1421\]"] {
            @apply bg-white border-gray-200 dark:border-white/10;
        }
        ...
        html:not(.dark) input {
            @apply bg-white text-gray-900 dark:bg-[#131928] dark:text-white dark:placeholder-gray-600 placeholder-gray-500;
        }
    </style>
</head>
                ...
                <form method="GET" class="space-y-4" id="filter-form">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
                                ...
                                <div class="relative w-full sm:w-auto">
                                    ...
                                    <div id="currency-picker"
                                        ...
                                        <div class="grid grid-cols-3 gap-1 max-h-80 overflow-y-auto">
                                            <button type="button" data-currency="EUR" data-symbol="€"
                                                data-rate="1.0"
                                                class="currency-opt text-xs py-2 px-2 rounded hover:bg-blue-600/50 transition text-center">€
                                                EUR</button>
                                            <button type="button" data-currency="USD" data-symbol="$"
                                                data-rate="1.10"
                                                class="currency-opt text-xs py-2 px-2 rounded hover:bg-blue-600/50 transition text-center">$
                                                USD</button>
                                            <button type="button" data-currency="GBP" data-symbol="£"
                                                data-rate="0.86"
                                                class="currency-opt text-xs py-2 px-2 rounded hover:bg-blue-600/50 transition text-center">£
                                                GBP</button>
                                            ...
                                            <button type="button" data-currency="NOK" data-symbol="kr"
                                                ...
                                                class="currency-opt text-xs py-2 px-2 rounded hover:bg-blue-600/50 transition text-center">kr
                                                NOK</button>
                                            <button type="button" data-currency="DKK" data-symbol="kr"
                                                data-rate="7.45"
                                                class="currency-opt text-xs py-2 px-2 rounded hover:bg-blue-600/50 transition text-center">kr
                                                DKK</button>
                                            <button type="button" data-currency="SGD" data-symbol="S$"
                                                data-rate="1.48"
                        ...
                        <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 items-stretch sm:items-center">
                            <button type="submit"
                                class="px-6 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-medium rounded-lg transition flex items-center justify-center gap-2 min-h-[44px] sm:min-h-auto">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                </svg>
                                Filteren
                                @php
                                    $activeFilters = collect([$search, $fromDate, $toDate, $minCost, $maxCost])
                                        ->filter()
                                        ->count();
                                @endphp
                                @if ($activeFilters > 0)
                                    <span
                                        class="ml-1 px-2 py-0.5 bg-cyan-700 rounded-full text-xs font-semibold">{{ $activeFilters }}</span>
                                @endif
                            </button>
                            @if ($search || $fromDate || $toDate || $minCost || $maxCost)
                                <a href="{{ request()->url() }}"
                                    class="px-6 py-2.5 bg-red-600/20 hover:bg-red-600/30 text-red-400 hover:text-red-300 text-sm font-medium rounded-lg transition border border-red-600/40 hover:border-red-600/60 flex items-center justify-center gap-2 min-h-[44px] sm:min-h-auto">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <!-- Export Buttons -->
                        <div
                            class="flex flex-col sm:flex-row gap-2 sm:gap-3 items-stretch sm:items-center border-t border-white/10 pt-3 mt-3 sm:border-t-0 sm:border-l sm:border-white/10 sm:pl-3 sm:pt-0 sm:mt-0">
                            <div class="relative group">
                                <button type="button"
                                    class="flex-1 sm:flex-none px-4 py-2.5 bg-green-600/20 hover:bg-green-600/30 text-green-400 hover:text-green-300 text-sm font-medium rounded-lg transition border border-green-600/40 hover:border-green-600/60 flex items-center justify-center gap-2 min-h-[44px] sm:min-h-auto">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div
                                    class="hidden group-hover:block absolute left-0 mt-0 sm:mt-2 bg-[#0f1421] border border-white/10 rounded-lg shadow-lg z-10 min-w-[200px]">
                                    <a href="{{ route('export.records-csv', array_merge(request()->query(), ['currency' => 'EUR'])) }}"
                                        class="block px-4 py-2.5 text-green-400 hover:bg-green-600/20 text-sm transition border-b border-white/10 last:border-b-0 first:rounded-t-lg last:rounded-b-lg flex items-center gap-2 min-h-[44px] sm:min-h-auto">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                        </svg>
                                        Alle records als CSV
                                    </a>
                                    <a href="{{ route('export.summary-csv', array_merge(request()->query(), ['group_by' => 'medewerker', 'currency' => 'EUR'])) }}"
                                        class="block px-4 py-2.5 text-green-400 hover:bg-green-600/20 text-sm transition border-b border-white/10 last:border-b-0 first:rounded-t-lg last:rounded-b-lg flex items-center gap-2 min-h-[44px] sm:min-h-auto">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div
                    ...
                    <div class="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
                        ...
                        <div class="text-right">
                            <p class="text-sm font-semibold text-cyan-400 mono"><span
                                    class="group-symbol">€</span><span
                                ...
                                @foreach ($records->sortByDesc('date')->values() as $record)
                                    <tr class="hover:bg-white/[0.02] transition"
                                        ...
                                        <td class="py-2.5 pr-4 text-gray-300 whitespace-nowrap text-xs">
                                            {{ $record->worker }}</td>
                                        <td
                                            class="py-2.5 pr-4 text-right text-gray-400 mono text-xs whitespace-nowrap">
