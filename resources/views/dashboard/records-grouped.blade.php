        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="flex justify-between items-center h-14">
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('dashboard') }}" class="text-gray-400 text-sm hover:text-gray-200 transition">Dashboard</a>
                    <span class="text-white/20 text-sm">/</span>
    ...
    <main class="max-w-7xl mx-auto px-6 lg:px-8 py-10">
                ...
                <div class="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ $records->count() }} {{ $records->count() === 1 ? 'record' : 'records' }}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-sm font-semibold text-cyan-400 mono">€ {{ number_format($records->sum('costs'), 2, ',', '.') }}</p>
                        <p class="text-xs text-gray-500">{{ number_format($records->sum('time'), 1, ',', '.') }}u totaal</p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-white/[0.04]">
                                <th class="pb-2 pr-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider whitespace-nowrap">Datum</th>
                                <th class="pb-2 pr-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider whitespace-nowrap">Actie</th>
                                <th class="pb-2 pr-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Omschrijving</th>
                                <th class="pb-2 pr-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider whitespace-nowrap">Medewerker</th>
                                <th class="pb-2 pr-4 text-right text-xs font-medium text-gray-600 uppercase tracking-wider whitespace-nowrap">Uren</th>
                                <th class="pb-2 text-right text-xs font-medium text-gray-600 uppercase tracking-wider whitespace-nowrap">Kosten</th>
                            </tr>
                        </thead>
                            ...
                            @foreach($records as $record)
                                <tr class="hover:bg-white/[0.02] transition">
                                    ...
                                    <td class="py-2.5 pr-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-500/15 text-blue-300 border border-blue-500/20">
                                            {{ $record->action }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="py-2.5 pr-4 text-gray-300 whitespace-nowrap text-xs">{{ $record->worker }}</td>
                                    <td class="py-2.5 pr-4 text-right text-gray-400 mono text-xs whitespace-nowrap">
                                        {{ number_format($record->time, 1, ',', '.') }}u
                                    </td>
                                    <td class="py-2.5 text-right font-medium text-cyan-400 mono text-xs whitespace-nowrap">
        ...
        @empty
            <div class="border border-white/8 bg-[#131928] rounded-xl p-16 text-center">
                ...
                <a href="{{ route('dashboard') }}" class="text-blue-400 text-sm hover:text-blue-300 transition mt-2 inline-block">
